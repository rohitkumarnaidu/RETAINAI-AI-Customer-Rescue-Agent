          ...
          <div className="flex-1 overflow-auto p-3 space-y-3 bg-[#F8F7F5]">
            ...
            {messages.map((m,idx)=>(
              <div key={idx} className={`flex ${m.role==='user'?'justify-end':'justify-start'}`}>
                <div className={`max-w-[84%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed whitespace-pre-wrap ${m.role==='user' ? 'bg-[#0F172A] text-white rounded-br-sm' : 'bg-white border border-slate-200 text-slate-800 rounded-bl-sm shadow-sm'}`}>
                  {m.content}
                  {m.evidence_ids && m.evidence_ids.length>0 && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {m.evidence_ids.slice(0,6).map(id=>(
                        <span key={id} className="text-[10px] font-mono bg-slate-900 text-white px-1.5 py-0.5 rounded-full">{id.slice(0,14)}</span>
                      ))}
                    </div>
                  )}
            ...
            {streamingText && (
              <div className="flex justify-start">
                <div className="max-w-[84%] bg-white border border-slate-200 rounded-2xl rounded-bl-sm px-3.5 py-2.5 text-sm leading-relaxed whitespace-pre-wrap shadow-sm">
                  {streamingText}<span className="inline-block w-1.5 h-3 bg-slate-900 ml-0.5 animate-pulse align-middle"/>
                </div>
              </div>
            )}
